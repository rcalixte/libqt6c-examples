...
static const char* FORMFILE = "src/libraries/designer/design.ui";

int main(int argc, char* argv[]) {
    QApplication* qapp = q_application_new(&argc, argv);

    QWidget* widget = q_widget_new2();
    q_widget_set_window_title(widget, "Qt 6 Designer Example");

    QVBoxLayout* layout = q_vboxlayout_new(widget);

    QFormBuilder* builder = q_formbuilder_new();
